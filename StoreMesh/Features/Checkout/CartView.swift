        ...
        NavigationStack {
            Group {
                if cart.lines.isEmpty {
                    ContentUnavailableView("Your cart is empty", systemImage: "cart", description: Text("Add products to continue."))
                } else {
                    List {
                        ForEach(cart.lines) { line in
                            ...
                            HStack {
                                VStack(alignment: .leading) {
                                    Text(product?.name ?? "Product").font(.headline)
                                    Text("\(line.quantity) × \(product?.currency ?? "") \(Double(product?.priceMinor ?? 0) / 100, specifier: "%.2f")")
                                ...
                                Stepper("\(line.quantity)", value: Binding(
                                    ...
                                    set: { value in
                                        Task {
                                            var next = cart
                                            if let index = next.lines.firstIndex(where: { $0.id == line.id }) {
                                                next.lines[index].quantity = value
                                            }
                                            next.lines.removeAll { $0.quantity < 1 }
                                            cart = next
                                            await onChange(next)
                                        }
                                    }
