...
private struct ProductArtwork: View { let product: Product; let height: CGFloat; private var assetName: String { switch product.sku ?? "" { case let sku where sku.hasPrefix("SM-LAMP") || sku.hasPrefix("SM-LITE"): return "halo-desk-lamp"; case let sku where sku.hasPrefix("SM-MUG") || sku.hasPrefix("SM-COFF") || sku.hasPrefix("SM-TEA"): return "cove-ceramic-mug"; case let sku where sku.hasPrefix("SM-CHRG") || sku.hasPrefix("SM-KEYB") || sku.hasPrefix("SM-CABL") || sku.hasPrefix("SM-HUB") || sku.hasPrefix("SM-STND"): return "arc-wireless-charger"; case let sku where sku.hasPrefix("SM-THRW") || sku.hasPrefix("SM-SHTS"): return "cloud-wool-throw"; case let sku where sku.hasPrefix("SM-TOTE"): return "canvas-market-tote"; case let sku where sku.hasPrefix("SM-BACK"): return "studio-backpack"; case let sku where sku.hasPrefix("SM-WLET"): return "common-leather-wallet"; case let sku where sku.hasPrefix("SM-UMBR"): return "rain-travel-umbrella"; case let sku where sku.hasPrefix("SM-SPKR"): return "lumen-portable-speaker"; case let sku where sku.hasPrefix("SM-HEAD"): return "ridge-headphones"; case let sku where sku.hasPrefix("SM-POT"): return "moss-plant-pot"; case let sku where sku.hasPrefix("SM-CNDL"): return "hearth-candle"; case let sku where sku.hasPrefix("SM-VASE"): return "north-ceramic-vase"; case let sku where sku.hasPrefix("SM-BASK"): return "woven-storage-basket"; default: return "halo-desk-lamp" } }; private var image: UIImage? { let paths = [Bundle.main.path(forResource: assetName, ofType: "png"), Bundle.main.path(forResource: "Catalog/\(assetName)", ofType: "png")].compactMap { $0 }; return paths.lazy.compactMap { UIImage(contentsOfFile: $0) }.first }; var body: some View { Group { if let image { Image(uiImage: image).resizable().scaledToFit().padding(6) } else { ZStack { RoundedRectangle(cornerRadius: 16).fill(Color.storeMeshBlue.opacity(0.1)); Image(systemName: "shippingbox.fill").font(.system(size: 40)).foregroundStyle(Color.storeMeshBlue) } } }.frame(maxWidth: .infinity).frame(height: height).clipShape(RoundedRectangle(cornerRadius: 16)) } }
